...
(define-public (stream-token-to
  ...
  (let (
    ...
    ;; Transfer tokens from sender to contract - for now, we'll handle this in a separate function
    ;; (try! (contract-call? token-contract token-trait transfer initial-balance contract-caller (as-contract tx-sender) none))
    (map-set streams current-stream-id stream)
    (var-set latest-stream-id (+ current-stream-id u1))
    (ok current-stream-id)
  )
